...
line_filtered, = ax1.plot([], [], label='Filtered & Normalized')
line_unfiltered, = ax1.plot([], [], label='Unfiltered & Normalized')
line_peaks, = ax1.plot([], [], 'ro', label='Detected Peaks', markersize=6)  # peaks line
...
ax1.set_title('Real-Time PPG Signal with Rolling Normalization and Peak Detection')
ax1.legend()
...
def update(frame):
    global Fs, start_time, ppg1_raw, times1_sec
    ...
    try:
        ...
        if line:
            ...
            if len(ppg1_raw) > max_buffer_length:
                times1_sec = times1_sec[-max_buffer_length:]
                ppg1_raw = ppg1_raw[-max_buffer_length:]

            # Update sampling frequency dynamically
            if len(times1_sec) > 2:
            ...
            if Fs and len(ppg1_raw) > max(order * 3, 10):
                ...
                if len(ppg1_filtered_signal) >= window_length:
                    recent_filtered = ppg1_filtered_signal[-window_length:]
                    recent_unfiltered = ppg1_raw[-window_length:]
                else:
                    recent_filtered = ppg1_filtered_signal
                    recent_unfiltered = ppg1_raw
                ...
                line_filtered.set_data(time_axis, ppg1_filtered_normalized)
                line_unfiltered.set_data(time_axis, ppg1_unfiltered_normalized)

                # Show last 10 seconds
                if time_axis[-1] > 10:
                    ax1.set_xlim(time_axis[-1] - 10, time_axis[-1])
                
                # Run AMPD for peak detection on a sliding window of filtered data
                if len(ppg1_filtered_normalized) >= ampd_window_length:
                    segment = ppg1_filtered_normalized[-ampd_window_length:]
                    ampd_peaks = ampdlib.ampd_fast(segment, window_length=ampd_window_length)

                    # Convert local peak indices to global indices
                    global_peak_indices = np.arange(len(ppg1_filtered_normalized) - ampd_window_length, 
                                                    len(ppg1_filtered_normalized))[ampd_peaks]
                    peak_times = time_axis[global_peak_indices]

                    # Plot peaks
                    peak_values = ppg1_filtered_normalized[global_peak_indices]
                    line_peaks.set_data(peak_times, peak_values)

                    # Compute heart rate if we have at least 2 peaks
                    if len(peak_times) >= 2:
                        rr_intervals = np.diff(peak_times)
                        avg_rr = np.mean(rr_intervals)
                        heart_rate = 60.0 / avg_rr
                        text_handle.set_text(f'HR: {heart_rate:.2f} BPM')
                    else:
                        text_handle.set_text('HR: -')
                else:
                    text_handle.set_text('HR: -')
                    line_peaks.set_data([], [])

        return line_filtered, line_unfiltered, line_peaks, text_handle
